...
export function FavoritesScroll({ favoritesVod, favoritesSeries, setSelectedSeries, setSelectedVod }: WatchingScrollProps) {
  ...
  const [watchingTab, setWatchingTab] = useState((favoritesSeries.length < 1) && (favoritesVod.length > 0) ? 1 : 0)
      ...
      <ScrollArea className="w-full whitespace-nowrap rounded-md">
        <div className="flex w-max space-x-4 pb-6 whitespace-nowrap rounded-md">
          <Fade duration={200} triggerOnce>
            {(watchingTab == 0 && favoritesSeries) && favoritesSeries!.map(series => {
              return (
              <div
            ...
            {(watchingTab == 1 && favoritesVod) && favoritesVod!.map(movie => {
              return (
                <div
                  className="flex flex-col hover:scale-95 transition gap-3 w-fit h-fit cursor-pointer relative"
                  key={movie.num}
