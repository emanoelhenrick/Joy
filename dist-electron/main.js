...
async function getLocalTmdbTrending() {
  const metadata = await getMetadata();
  const currentPlaylist = metadata.currentPlaylist.name;
  const trending = await main$1.readAsync(path$5.join(getPlaylistFolderPath(currentPlaylist), "trending.json"), "json");
  return trending || [];
}
  ...
  trendingWorkerPromise = (async () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.on("message", async (msg) => {
        ...
        if (msg.isSuccess) {
          await main$1.writeAsync(path$5.join(getPlaylistFolderPath(currentPlaylist), "trending.json"), msg.tmdbData);
