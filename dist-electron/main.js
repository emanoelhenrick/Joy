...
async function GA({ apiKey: i, playlist: e }) {
  let t = [];
  const s = new Km.MovieDb(i);
  try {
    t = (await s.trending({ language: "pt", media_type: "movie", time_window: "week" })).results;
  } catch {
    t = (await s.trending({ language: "pt", media_type: "movie", time_window: "week" })).results;
  }
  ...
  return await Promise.all(
    t.map(async (l) => {
      const p = l.title + l.release_date.split("-")[0], f = o.search(p).map((m) => m.item);
      if (f.length > 0) {
        const m = await s.movieImages({ id: l.id });
        c.push({ ...l, matches: f, images: m });
      }
    })
  ), c;
}
