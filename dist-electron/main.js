import require$$3$1, { app, ipcMain, BrowserWindow } from "electron";
...
function requireFs() {
  ...
  const promisify2 = requirePromisify();
...
function requireRemove() {
  ...
  hasRequiredRemove = 1;
  const fs = requireFs();
  const validate2 = requireValidate();
  const validateInput = (methodName, path2) => {
    const methodSignature = `${methodName}([path])`;
    validate2.argument(methodSignature, "path", path2, ["string", "undefined"]);
  };
  ...
  remove.validateInput = validateInput;
  remove.sync = removeSync;
  remove.async = removeAsync;
  return remove;
}
var hasRequiredDir;
function requireDir() {
  ...
  hasRequiredDir = 1;
  const pathUtil = path$1;
  const fs = requireFs();
  const modeUtil = requireMode();
  const validate2 = requireValidate();
  const remove2 = requireRemove();
  const validateInput = (methodName, path2, criteria) => {
    const methodSignature = `${methodName}(path, [criteria])`;
    validate2.argument(methodSignature, "path", path2, ["string"]);
    validate2.options(methodSignature, "criteria", criteria, {
      empty: ["boolean"],
      mode: ["string", "number"]
    });
  };
  ...
  dir.validateInput = validateInput;
  dir.sync = dirSync;
  dir.createSync = createBrandNewDirectorySync;
  dir.async = dirAsync;
  dir.createAsync = createBrandNewDirectoryAsync;
  return dir;
}
...
function requireWrite() {
  ...
  const fs = requireFs();
  const validate2 = requireValidate();
  const dir2 = requireDir();
  const validateInput = (methodName, path2, data, options) => {
    const methodSignature = `${methodName}(path, data, [options])`;
    validate2.argument(methodSignature, "path", path2, ["string"]);
    validate2.argument(methodSignature, "data", data, [
      "string",
      "buffer",
      "object",
      "array"
    ]);
    validate2.options(methodSignature, "options", options, {
      mode: ["string", "number"],
      atomic: ["boolean"],
      jsonIndent: ["number"]
    });
  ...
  write.validateInput = validateInput;
...
function requireAppend() {
  ...
  hasRequiredAppend = 1;
  const fs = requireFs();
  const write2 = requireWrite();
  const validate2 = requireValidate();
  const validateInput = (methodName, path2, data, options) => {
    const methodSignature = `${methodName}(path, data, [options])`;
    validate2.argument(methodSignature, "path", path2, ["string"]);
    validate2.argument(methodSignature, "data", data, ["string", "buffer"]);
    validate2.options(methodSignature, "options", options, {
      mode: ["string", "number"]
    });
  };
  ...
  append.validateInput = validateInput;
  append.sync = appendSync;
  append.async = appendAsync;
  return append;
}
var file = {};
var hasRequiredFile;
function requireFile() {
  ...
  file.validateInput = validateInput;
  file.sync = fileSync;
  file.async = fileAsync;
  return file;
}
var find = {};
var tree_walker = {};
...
function requireInspect() {
  ...
  inspect.supportedChecksumAlgorithms = supportedChecksumAlgorithms;
  inspect.symlinkOptions = symlinkOptions;
  inspect.validateInput = validateInput;
  inspect.sync = inspectSync;
  inspect.async = inspectAsync;
  return inspect;
}
var list = {};
var hasRequiredList;
function requireList() {
  ...
  list.validateInput = validateInput;
  list.sync = listSync;
  list.async = listAsync;
  return list;
}
var hasRequiredTree_walker;
function requireTree_walker() {
  if (hasRequiredTree_walker) return tree_walker;
  hasRequiredTree_walker = 1;
  const fs = require$$0$1;
  const pathUtil = path$1;
  const inspect2 = requireInspect();
  requireList();
...
function requireMatcher() {
  ...
  const Minimatch = requireMinimatch().Minimatch;
  ...
  matcher.create = (basePath, patterns, ignoreCase) => {
...
function requireFind() {
  ...
  hasRequiredFind = 1;
  const pathUtil = path$1;
  const treeWalker = requireTree_walker();
  const inspect2 = requireInspect();
  const matcher2 = requireMatcher();
  const validate2 = requireValidate();
  const validateInput = (methodName, path2, options) => {
    const methodSignature = `${methodName}([path], options)`;
    validate2.argument(methodSignature, "path", path2, ["string"]);
  ...
  const findSync = (path2, options) => {
    ...
    const matchesAnyOfGlobs = matcher2.create(
      path2,
      options.matching,
      options.ignoreCase
    );
  ...
  find.validateInput = validateInput;
  find.sync = findSyncInit;
  find.async = findAsyncInit;
  return find;
}
var inspect_tree = {};
var hasRequiredInspect_tree;
function requireInspect_tree() {
  if (hasRequiredInspect_tree) return inspect_tree;
  hasRequiredInspect_tree = 1;
  const crypto = require$$0$2;
  const pathUtil = path$1;
  const inspect2 = requireInspect();
  requireList();
  ...
  inspect_tree.validateInput = validateInput;
...
function requireExists() {
  ...
  exists.validateInput = validateInput;
  exists.sync = existsSync;
  exists.async = existsAsync;
  return exists;
}
var hasRequiredCopy;
function requireCopy() {
  ...
  const fs = requireFs();
  const dir2 = requireDir();
  const exists2 = requireExists();
  const inspect2 = requireInspect();
  const write2 = requireWrite();
  const matcher2 = requireMatcher();
  const fileMode = requireMode();
  const treeWalker = requireTree_walker();
  const validate2 = requireValidate();
  const validateInput = (methodName, from, to, options) => {
    const methodSignature = `${methodName}(from, to, [options])`;
    validate2.argument(methodSignature, "from", from, ["string"]);
    validate2.argument(methodSignature, "to", to, ["string"]);
    validate2.options(methodSignature, "options", options, {
  ...
  const parseOptions = (options, from) => {
    ...
    if (opts.matching) {
      parsedOptions.allowedToCopy = matcher2.create(
        from,
        opts.matching,
        opts.ignoreCase
      );
  ...
  const copyFileSync = (srcPath, destPath, mode2, context) => {
    ...
    try {
      fs.writeFileSync(destPath, data, { mode: mode2, flag: "wx" });
    } catch (err) {
      if (err.code === "ENOENT") {
        write2.sync(destPath, data, { mode: mode2 });
      } else if (err.code === "EEXIST") {
        if (canOverwriteItSync(context)) {
          fs.writeFileSync(destPath, data, { mode: mode2 });
  ...
  const copyItemSync = (srcPath, srcInspectData, destPath, opts) => {
    ...
    if (srcInspectData.type === "dir") {
      dir2.createSync(destPath, { mode: mode2 });
  ...
  const copyFileAsync = (srcPath, destPath, mode2, context, runOptions) => {
    return new Promise((resolve, reject) => {
      ...
      const readStream2 = fs.createReadStream(srcPath);
      const writeStream = fs.createWriteStream(destPath, { mode: mode2, flags });
  ...
  const copyItemAsync = (srcPath, srcInspectData, destPath, opts) => {
    ...
    const mode2 = fileMode.normalizeFileMode(srcInspectData.mode);
    if (srcInspectData.type === "dir") {
      return dir2.createAsync(destPath, { mode: mode2 });
    } else if (srcInspectData.type === "file") {
  ...
  copy.validateInput = validateInput;
  copy.sync = copySync;
  copy.async = copyAsync;
  return copy;
}
var move = {};
var hasRequiredMove;
function requireMove() {
  ...
  move.validateInput = validateInput;
  move.sync = moveSync;
  move.async = moveAsync;
  return move;
}
var read = {};
var hasRequiredRead;
function requireRead() {
  ...
  const validateInput = (methodName, path2, returnAs) => {
    ...
    validate2.argument(methodSignature, "path", path2, ["string"]);
    validate2.argument(methodSignature, "returnAs", returnAs, [
      "string",
      "undefined"
    ]);
  ...
  read.validateInput = validateInput;
  read.sync = readSync;
  read.async = readAsync;
  return read;
}
var rename = {};
...
function requireRename() {
  ...
  rename.validateInput = validateInput;
  rename.sync = renameSync;
  rename.async = renameAsync;
  return rename;
}
var symlink = {};
...
function requireSymlink() {
  ...
  const validateInput = (methodName, symlinkValue, path2) => {
    ...
    validate2.argument(methodSignature, "symlinkValue", symlinkValue, ["string"]);
    validate2.argument(methodSignature, "path", path2, ["string"]);
  };
  ...
  symlink.validateInput = validateInput;
  symlink.sync = symlinkSync;
  symlink.async = symlinkAsync;
  return symlink;
}
var streams = {};
...
function requireStreams() {
  ...
  const fs = require$$0$1;
  streams.createWriteStream = fs.createWriteStream;
  streams.createReadStream = fs.createReadStream;
  return streams;
}
var tmp_dir = {};
var hasRequiredTmp_dir;
function requireTmp_dir() {
  if (hasRequiredTmp_dir) return tmp_dir;
  hasRequiredTmp_dir = 1;
  const pathUtil = path$1;
  const os = require$$1;
  const crypto = require$$0$2;
  const dir2 = requireDir();
...
var jetpack;
var hasRequiredJetpack;
function requireJetpack() {
  if (hasRequiredJetpack) return jetpack;
  hasRequiredJetpack = 1;
  const util = require$$1$1;
  const pathUtil = path$1;
  const append3 = requireAppend();
  const dir2 = requireDir();
  const file2 = requireFile();
  const find2 = requireFind();
  const inspect2 = requireInspect();
  const inspectTree = requireInspect_tree();
  const copy2 = requireCopy();
  const exists2 = requireExists();
  const list2 = requireList();
  const move2 = requireMove();
  const read2 = requireRead();
  const remove2 = requireRemove();
  const rename2 = requireRename();
  const symlink2 = requireSymlink();
  const streams2 = requireStreams();
  const tmpDir = requireTmp_dir();
  const write2 = requireWrite();
  const jetpackContext = (cwdPath) => {
    ...
    const api = {
      ...
      append: (path2, data, options) => {
        append3.validateInput("append", path2, data, options);
        append3.sync(resolvePath(path2), data, options);
      },
      appendAsync: (path2, data, options) => {
        append3.validateInput("appendAsync", path2, data, options);
        return append3.async(resolvePath(path2), data, options);
      },
      ...
      createWriteStream: (path2, options) => {
        return streams2.createWriteStream(resolvePath(path2), options);
      },
      createReadStream: (path2, options) => {
        return streams2.createReadStream(resolvePath(path2), options);
      },
      dir: (path2, criteria) => {
...
function requireMain$1() {
  ...
  hasRequiredMain$1 = 1;
  const jetpack2 = requireJetpack();
  main$1 = jetpack2();
  return main$1;
}
var mainExports$1 = requireMain$1();
const SessionDataDir = app.getPath("sessionData");
const PLAYLIST_DIR = path$1.join(SessionDataDir, "Playlists");
const META_PATH = path$1.join(PLAYLIST_DIR, "meta.json");
const getPlaylistFolderPath = (playlistName) => path$1.join(SessionDataDir, `Playlists/${playlistName}`);
const getUserDataPath = (playlistName, profile) => path$1.join(SessionDataDir, `Playlists/${playlistName}/user/${profile}.json`);
const getVodPath = (playlistName) => path$1.join(SessionDataDir, `Playlists/${playlistName}/vod.json`);
const getSeriesPath = (playlistName) => path$1.join(SessionDataDir, `Playlists/${playlistName}/series.json`);
const getLivePath = (playlistName) => path$1.join(SessionDataDir, `Playlists/${playlistName}/live.json`);
async function getMetadata() {
  const metadata = await mainExports$1.readAsync(META_PATH, "json");
  if (!metadata) {
    const newMeta = { currentPlaylist: { name: "", profile: "" }, playlists: [] };
    await mainExports$1.writeAsync(META_PATH, newMeta);
    return newMeta;
  }
  return metadata;
}
...
async function updateVod(name) {
  ...
  await mainExports$1.writeAsync(getVodPath(name), data);
  return data;
}
async function updateSeries(name) {
  ...
  if (playlistResponse.data.length === 0 || categoriesResponse.data.length === 0) return;
  const data = { playlist: playlistResponse.data, categories: categoriesResponse.data };
  await mainExports$1.writeAsync(getSeriesPath(name), data);
  return data;
}
async function updateLive(name) {
  ...
  if (playlistResponse.data.length === 0 || categoriesResponse.data.length === 0) return;
  const data = { playlist: playlistResponse.data, categories: categoriesResponse.data };
  await mainExports$1.writeAsync(getLivePath(name), data);
  return data;
}
async function authenticateUser(url) {
  try {
...
async function addPlaylistToMeta(playlistInfo) {
  ...
  return await mainExports$1.writeAsync(META_PATH, metadata);
}
async function getLocalVodPlaylist(playlistName) {
  const VOD_PATH = getVodPath(playlistName);
  const VodPlaylist = await mainExports$1.readAsync(VOD_PATH, "json");
  return VodPlaylist;
}
async function getLocalSeriesPlaylist(playlistName) {
  const SeriesPlaylist = await mainExports$1.readAsync(getSeriesPath(playlistName), "json");
  return SeriesPlaylist;
}
async function getLocalLivePlaylist(playlistName) {
  const LivePlaylist = await mainExports$1.readAsync(getLivePath(playlistName), "json");
  return LivePlaylist;
}
async function getPlaylistInfo(playlistName) {
  const meta = await mainExports$1.readAsync(META_PATH, "json");
  return meta.playlists.find((p) => p.name == playlistName);
}
async function getVodInfo(url) {
  if (!url) return;
  const res = await axios.get(url);
  if (!res.data || res.status !== 200) return;
  return res.data;
}
async function getSerieInfo(url) {
  if (!url) return;
  const res = await axios.get(url);
  if (!res.data || res.status !== 200) return;
  return res.data;
...
async function getUserData(profile) {
  ...
  let userData = await mainExports$1.readAsync(getUserDataPath(currentPlaylist.name, profile), "json");
  if (!userData) {
    userData = { vod: [], series: [], live: [] };
    await mainExports$1.writeAsync(USERDATA_PATH, userData);
  }
  return userData;
}
async function updateUserData(data) {
  ...
  await mainExports$1.writeAsync(USERDATA_PATH, data);
  return data;
}
async function changeCurrentPlaylist(playlistName) {
  ...
  await mainExports$1.writeAsync(META_PATH, metadata);
  return true;
}
async function updatedAtPlaylist(playlistName) {
  ...
  return await mainExports$1.writeAsync(META_PATH, meta);
}
async function createProfile(profile) {
  ...
  await mainExports$1.writeAsync(META_PATH, metadata);
  let userData = await mainExports$1.readAsync(getUserDataPath(metadata.currentPlaylist.name, profile), "json");
  if (!userData) {
    userData = { vod: [], series: [], live: [] };
    await mainExports$1.writeAsync(USERDATA_PATH, userData);
    return true;
...
async function renameProfile({ profile, newName }) {
  ...
  await mainExports$1.writeAsync(META_PATH, meta);
  return await mainExports$1.renameAsync(USERDATA_PATH, `${newName}.json`);
}
async function removeProfile(profile) {
  ...
  await mainExports$1.writeAsync(META_PATH, metadata);
  const USERDATA_PATH = getUserDataPath(metadata.currentPlaylist.name, profile);
  await mainExports$1.removeAsync(USERDATA_PATH);
}
async function removePlaylist(playlistName) {
  ...
  await mainExports$1.removeAsync(getPlaylistFolderPath(playlistName));
  await mainExports$1.writeAsync(META_PATH, metadata);
  return metadata;
}
...
async function editPlaylistInfo({ playlistName, newPlaylistInfo }) {
  ...
  metadata.playlists = updatedPlaylists;
  await mainExports$1.renameAsync(getPlaylistFolderPath(playlistName), newPlaylistInfo.name);
  return await mainExports$1.writeAsync(META_PATH, metadata);
}
...
async function getLocalTmdbTrending() {
  const metadata = await getMetadata();
  const currentPlaylist = metadata.currentPlaylist.name;
  const trending = await mainExports$1.readAsync(path$1.join(getPlaylistFolderPath(currentPlaylist), "trending.json"), "json");
  return trending || [];
}
const __filename = fileURLToPath(import.meta.url);
const __dirname$1 = path$1.dirname(__filename);
let trendingWorkerPromise = null;
async function runFetchTmdbTrendingInWorker(apiKey, win2) {
  ...
  trendingWorkerPromise = (async () => {
    ...
    return new Promise((resolve, reject) => {
      const worker = new Worker(
        path$1.resolve(__dirname$1, "./fetchTmdbTrending.worker.js"),
        { workerData: { apiKey, vodPlaylist } }
      );
      worker.on("message", async (msg) => {
        trendingWorkerPromise = null;
        if (msg.isSuccess) {
          await mainExports$1.writeAsync(path$1.join(getPlaylistFolderPath(currentPlaylist), "trending.json"), msg.tmdbData);
...
function CoreControllers(win2) {
  ...
  ipcMain.handle("update-current-playlist", async (_event) => {
    ...
    if (response.isSuccess) await runFetchTmdbTrendingInWorker(TMDB_API_KEY, win2);
    return response;
  });
  ...
  (function(module, exports) {
    ...
    function createWritableStdioStream(fd2) {
      ...
      switch (tty_wrap.guessHandleType(fd2)) {
        ...
        case "FILE":
          var fs = require$$0$1;
          stream22 = new fs.SyncWriteStream(fd2, { autoClose: false });
          stream22._type = "fs";
          break;
        case "PIPE":
        case "TCP":
...
function createWindow() {
  win = new BrowserWindow({
    icon: path$2.join(process.env.VITE_PUBLIC, "icons/icon.png"),
    webPreferences: {
